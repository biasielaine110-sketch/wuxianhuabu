 ...
 * Serverless 缓冲转发，并从 x-deepwhite-key 取 Key（避免部署保护把 Authorization 换成 JWT）。
 */
...
function pickApiKey(req) {
  const custom = normalizeKey(req.headers['x-deepwhite-key']);
  ...
  const env = normalizeKey(process.env.DEEPWHITE_API_KEY || '');
...
async function handler(req, res) {
  ...
  if (method === 'OPTIONS') {
    ...
    res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization, x-deepwhite-key');
  ...
  if (!apiKey) {
    ...
    res.end(
      JSON.stringify({
        error: {
          message:
            '未提供可用的 DeepWhite API Key（已忽略 Vercel 部署保护 JWT）。请在「设置 → API → DeepWhite」填写并保存。',
          type: 'Unauthorized',
        },
      })
    );
  ...
  try {
    ...
    if (!res.headersSent) {
      ...
      res.end(
        JSON.stringify({
          error: 'deepwhite_upstream_read_failed',
          message: e instanceof Error ? e.message : String(e),
        })
      );
    }
