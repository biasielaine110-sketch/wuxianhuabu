  ...
  const classifyError = useCallback((rawError: string, node: CanvasNode) => {
    ...
    if (msg.includes('quota') || msg.includes('rate') || msg.includes('429') || msg.includes('too many requests') || msg.includes('resource exhausted') || msg.includes('insufficient_user_quota')) {
      return {
        ...
        fixes: [
          ...
          { label: '切换免费模型', action: () => handleUpdateNode(node.id, { model: node.type === 'chat' ? 'gemini-2.5-flash' : 'qwen-image-3.0', error: undefined }) },
        ]
      };
    }
    ...
    return {
      ...
      fixes: [
        ...
        { label: '🔄 切换模型', action: () => handleUpdateNode(node.id, { model: node.type === 'chat' ? 'gemini-2.5-flash' : 'qwen-image-3.0', error: undefined }) },
      ]
    };
  }, [handleUpdateNode]);
  ...
  return (
    ...
    {nodeContextMenu && canvasMode !== 'audit' && (
      ...
      {/* 修复图片资产按钮 - 画布右上角（看图模式下隐藏）。
          ...
          onPointerDown / onMouseDown / onClick 全部 stopPropagation 防止
          canvas-container 的 onPointerDown 长按计时 / 框选状态机把 click 吞掉。 */}
      <button
        type="button"
